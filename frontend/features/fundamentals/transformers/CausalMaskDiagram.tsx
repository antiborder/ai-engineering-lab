      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="bg-white rounded-md border border-neutral-200 w-full h-auto">
        ...
        {TOKENS.map((_, i) => {
          ...
          return allowed ? (
            ...
            <text key={`x-${i}`} x={x} y={arcHeight + 10} fontSize={15} textAnchor="middle" fill="#dc2626">
              ×
            </text>
          );
        })}
        ...
        {TOKENS.map((tok, i) => {
          ...
          return (
            <g key={i}>
              ...
              />
              <text x={x} y={rowY + boxH / 2 + 4} fontSize={14} textAnchor="middle" fill={allowed ? "#3f3f46" : "#a3a3a3"}>
                {tok}
              </text>
            </g>
          );
        ...
        <text x={qx} y={rowY + boxH + 18} fontSize={14} textAnchor="middle" fill={CYAN} fontWeight={700}>
          query token
        </text>
      </svg>
