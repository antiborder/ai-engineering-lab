    ...
    <div className="w-full max-w-[460px] mx-auto">
      ...
      >
        <g opacity={op("residual")}>
          ...
          />
          <text x={(inX + addX) / 2} y={cy - 56} fontSize={11} textAnchor="middle" fill={ORANGE}>
            + old_x (residual)
          </text>
        </g>
        ...
        <g opacity={op("input")}>
          ...
          <text x={inX} y={cy + 4} fontSize={9} textAnchor="middle" fill="#3f3f46">old_x</text>
        </g>

        <line x1={inX + 22} y1={cy} x2={subX - 42} y2={cy} stroke={NEUTRAL} strokeWidth={2} opacity={Math.min(op("input"), op("sublayer"))} />
        <g opacity={op("sublayer")}>
          <rect x={subX - 42} y={cy - 18} width={84} height={36} rx={6} fill="rgba(8,145,178,0.12)" stroke={CYAN} strokeWidth={sw("sublayer", 2)} />
          <text x={subX} y={cy + 5} fontSize={11} textAnchor="middle" fill="#3f3f46">{sublayerLabel}</text>
        </g>

        <line x1={subX + 42} y1={cy} x2={addX - 15} y2={cy} stroke={NEUTRAL} strokeWidth={2} opacity={Math.min(op("sublayer"), op("residual"))} />
        <g opacity={op("residual")}>
        ...
        <g opacity={op("norm")}>
          ...
          <text x={normX} y={cy + 5} fontSize={10} textAnchor="middle" fill="#3f3f46">LayerNorm</text>
        </g>
