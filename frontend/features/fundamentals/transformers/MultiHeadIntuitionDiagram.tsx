    ...
    <svg viewBox={`0 0 ${width} 90`} className="w-full h-auto">
      ...
      {TOKENS.map((tok, i) => (
        <g key={i}>
          ...
          <text x={cellX(i)} y={rowY + boxH / 2 + 4} fontSize={9.5} textAnchor="middle" fill="#3f3f46">
            {tok}
          </text>
        </g>
      ))}
  ...
  return (
    <div className="w-full max-w-[460px] mx-auto space-y-3">
      <div className="rounded-md border border-neutral-200 bg-white p-2">
        <div className="text-[10px] font-semibold text-cyan-700 mb-1">Head A (illustrative): nearby words</div>
        <ArcRow pairs={NEARBY_PAIRS} color={CYAN} />
      </div>
      <div className="rounded-md border border-neutral-200 bg-white p-2">
        <div className="text-[10px] font-semibold text-violet-700 mb-1">Head B (illustrative): what a word refers to</div>
