  ...
  return (
    <div className="w-full max-w-[300px] mx-auto">
      <svg viewBox={`0 0 ${width} ${height}`} className="bg-white rounded-md border border-neutral-200 w-full h-auto">
        ...
        <text x={midX} y={softmaxY + softmaxH / 2 + 4} fontSize={14} textAnchor="middle" fill="#3f3f46">
          Softmax
        </text>
        ...
        <text x={blockX + 10} y={blockTop + 14} fontSize={14} fontWeight={700} fill="#525252">
          Transformer Block
        </text>
        <text x={blockX + 10} y={blockTop + 25} fontSize={14} fill="#737373">
          × N (this app: up to 3)
        </text>

        <rect x={sublayerX} y={ffnY} width={sublayerW} height={sublayerH} rx={6} fill="rgba(8,145,178,0.1)" stroke={CYAN} strokeWidth={2} />
        <text x={midX} y={ffnY + 18} fontSize={14} textAnchor="middle" fill="#3f3f46">
          Feed-Forward
        </text>
        <text x={midX} y={ffnY + 33} fontSize={14} textAnchor="middle" fill={ORANGE}>
          + residual, norm
        </text>
        ...
        <text x={midX} y={attnY + 18} fontSize={14} textAnchor="middle" fill="#3f3f46">
          Masked Self-Attention
        </text>
        <text x={midX} y={attnY + 33} fontSize={14} textAnchor="middle" fill={ORANGE}>
          + residual, norm
        </text>
        ...
        <text x={midX} y={tokenY + tokenH / 2 + 4} fontSize={14} textAnchor="middle" fill="#3f3f46">
          Token + Position
        </text>
