  ...
  return (
    <div className="w-full max-w-[460px] mx-auto">
      <svg viewBox={`0 0 ${width} ${height}`} className="bg-white rounded-md border border-neutral-200 w-full h-auto">
        ...
        <text x={hidX} y={16} fontSize={14} textAnchor="middle" fill={CYAN}>
          ⋮ {HIDDEN_TOTAL} total
        </text>

        <text x={(inX + hidX) / 2} y={16} fontSize={14} textAnchor="middle" fill={PURPLE}>
          W₁, b₁
        </text>
        <text x={(hidX + outX) / 2} y={16} fontSize={14} textAnchor="middle" fill={PURPLE}>
          W₂, b₂
        </text>

        <text x={inX} y={height - 6} fontSize={14} textAnchor="middle" fill="#525252">
          {D_MODEL} dims (x)
        </text>
        <text x={hidX} y={height - 6} fontSize={14} textAnchor="middle" fill="#525252">
          {HIDDEN_TOTAL} hidden, ReLU
        </text>
        <text x={outX} y={height - 6} fontSize={14} textAnchor="middle" fill="#525252">
          {D_MODEL} dims
        </text>
      </svg>
    </div>
