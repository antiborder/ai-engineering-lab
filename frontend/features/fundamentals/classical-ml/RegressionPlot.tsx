...
export function RegressionPlot({
  ...
  return (
    ...
    >
      ...
      {xTicks.map((t, i) => (
        <text key={`xt-${i}`} x={sx(t)} y={PAD.top + PLOT_H + 13} fontSize={14} textAnchor="middle" fill="rgba(23,23,23,0.65)">
          {t.toFixed(1)}
        </text>
      ))}
      {yTicks.map((t, i) => (
        <text key={`yt-${i}`} x={PAD.left - 5} y={sy(t) + 3} fontSize={14} textAnchor="end" fill="rgba(23,23,23,0.65)">
          {t.toFixed(1)}
        </text>
      ))}
      <text x={WIDTH - 2} y={PAD.top + PLOT_H + 13} fontSize={14} textAnchor="end" fill="rgba(23,23,23,0.75)">
        x
      </text>
      <text x={9} y={PAD.top + 8} fontSize={14} textAnchor="start" fill="rgba(23,23,23,0.75)">
        y
      </text>
    </svg>
    </div>
